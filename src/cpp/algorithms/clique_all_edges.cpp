...
vi get_neighbours_by_condition(PolygonGraph& graph, int source, function<bool(Polygon,Polygon)> condition) {
    vector<bool> vis (graph.get_mx_node_id() + 1);
    assert(graph.does_node_exist(source));
    queue<int> q; q.push(source);
    vector<int> res;
    Polygon src_pol = graph.get_polygon(source);
    while(!q.empty()) {
        int node = q.front(); q.pop();
        if(vis[node]) continue;
        vis[node] = true;
        if(!condition(src_pol, graph.get_polygon(node))) continue;
...
vector<vector<pair<OUTPUT_TYPE,OUTPUT_TYPE>>> get_clique_decomposition(vector<pair<int,int>> boundary_points, vector<vector<pair<int,int>>> holes_points) {
    ...
    PolygonGraph graph (triangulation);
    ...
    auto can_see_each_other_relaxed = [&](Polygon t1, Polygon t2) -> bool {
        Polygon pol = get_convex_hull_of_polygons({t1,t2});
        number_of_times++;
        TIME start = NOW();
        auto bbox = pol.bbox();
    ...
    for(auto& node : graph.get_nodes()) {
        TIME start = NOW();
        vi nodes = get_neighbours_by_condition(graph, node, can_see_each_other_relaxed);
        number++;
        debug(number);
        debug(sz(nodes), sz(triangulation));
    ...
    debug(number_of_times, number_of_times_2);
    return cgal2pysolution(polygons);*/
}

PYBIND11_MODULE(clique_all_edges, m) {
    m.doc() = "clique_all_edges";
