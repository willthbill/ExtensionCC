...
vector<vector<pair<OUTPUT_TYPE,OUTPUT_TYPE>>> get_convex_cover(vector<pair<int,int>> boundary_points, vector<vector<pair<int,int>>> holes_points) {
    ...
    CHGraph chgraph_extension (input_polygon, extension_triangulation, "delaunay_fully_visible_bfs_exact_nohelp_benchmark");
    ...
    cout << "\n\n\n";
    cout << "delaunay triangulation variant with BFS visibility graph and chalupa" << endl;

    return res;
}

PYBIND11_MODULE(fully_visible_delaunay, m) {
    m.doc() = "fully_visible_delaunay";
