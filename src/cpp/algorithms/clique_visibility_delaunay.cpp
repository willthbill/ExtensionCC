...
vector<vector<pair<OUTPUT_TYPE,OUTPUT_TYPE>>> get_clique_decomposition(vector<pair<int,int>> boundary_points, vector<vector<pair<int,int>>> holes_points) {
    ...
    CHGraph chgraph (input_polygon, triangulation, "delaunay_benchmark");
    ...
    auto polygons = cliquecover.get_cliques_vcc();
    TIME end_clique = NOW();
    ...
    cout << "  size of graph (|V|, |E|): " << "(" << cliquecover.n() << ", " << cliquecover.m() << ")" << endl;
    ...
    cout << "\n\n\n";
    cout << "delaunay triangulation variant with full visibility graph and chalupa" << endl;

    return cgal2pysolution(polygons);
}

PYBIND11_MODULE(clique_visibility_delaunay, m) {
    m.doc() = "clique_visibility_delaunay";
